...
def main():
    ...
    client = DisperserClientV2Full(
        ...
        payment_config=PaymentConfig(
            price_per_symbol=447000000, min_num_symbols=4096  # 447 gwei per symbol
        ),
    ...
    try:
        ...
        if (
            payment_state
            and hasattr(payment_state, "quorum_reservations")
            and payment_state.quorum_reservations
        ):
    ...
    if not has_reservation:
        try:
            ...
            if hasattr(simple_state, "reservation") and simple_state.HasField("reservation"):
                reservation = simple_state.reservation
                if reservation.start_timestamp > 0 and reservation.end_timestamp > 0:
    ...
    try:
        ...
        status, blob_key = client.disperse_blob(
            encoded_data, blob_version=0, quorum_ids=[0, 1]  # Request multiple quorums
        )
