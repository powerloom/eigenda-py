    ...
    try:
        ...
        if payment_info["payment_type"] == "reservation":
            ...
            if payment_info["reservation_details"]:
                ...
                print(f"  • Started: {datetime.fromtimestamp(details['start_timestamp']).strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"  • Expires: {datetime.fromtimestamp(details['end_timestamp']).strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                print(f"  • Per day: {daily_capacity:,} symbols (~{daily_capacity * 31 / (1024*1024):.2f} MB)")
                print(f"  • Per month: {monthly_capacity:,} symbols (~{monthly_capacity * 31 / (1024*1024):.2f} MB)")
                ...
                status, blob_key = client.disperse_blob(
                    data=test_data,
                    blob_version=0,
                    quorum_numbers=details['quorum_numbers'][:2] if len(details['quorum_numbers']) >= 2 else [0]
                )
            ...
            if payment_info["payment_type"] == "on_demand":
                ...
                print(f"Current balance: {payment_info['onchain_balance']} wei ({payment_info['onchain_balance']/1e18:.4f} ETH)")
            else:
