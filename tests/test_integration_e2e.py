...
from eigenda.utils.serialization import hash_payment_metadata, calculate_blob_key
...
from eigenda.config import get_network_config, NetworkConfig
from eigenda.core.types import PaymentMetadata
...
class TestEndToEndFlow:
    ...
    def test_complete_dispersal_flow(self, mock_signer, mock_grpc_responses):
        ...
        with patch('grpc.insecure_channel'), \
             patch('eigenda.grpc.disperser.v2.disperser_v2_pb2_grpc.DisperserStub') as mock_stub_class:
    ...
    def test_network_configuration_integration(self):
        ...
        # Test Sepolia
        with patch.dict(os.environ, {'EIGENDA_DISPERSER_HOST': 'disperser-testnet-sepolia.eigenda.xyz'}):
        ...
        # Test Holesky
        with patch.dict(os.environ, {'EIGENDA_DISPERSER_HOST': 'disperser-testnet-holesky.eigenda.xyz'}):
            config = get_network_config()
    ...
    def test_signature_verification_flow(self, mock_signer):
        ...
        with patch('grpc.insecure_channel'), \
            ...
            def capture_disperse(request, **kwargs):  # Accept any kwargs for timeout etc
                ...
                captured_signature = request.signature  # signature is at request level, not in blob_header
                return disperser_v2_pb2.DisperseBlobReply(
                    result=disperser_v2_pb2.BlobStatus.QUEUED,
...
class TestErrorScenarios:
    ...
    def test_network_error_handling(self, mock_signer):
        ...
        with patch('grpc.insecure_channel') as mock_channel:
            ...
            with patch('eigenda.grpc.disperser.v2.disperser_v2_pb2_grpc.DisperserStub') as mock_stub_class:
                mock_stub = Mock()
                mock_stub_class.return_value = mock_stub
