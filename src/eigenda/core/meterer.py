...
def is_within_advance_window(
    ...
    reservation_start_s: int,
    advance_window_s: int
) -> bool:
    """
...
def validate_reservations(
    ...
    for quorum_id in quorum_numbers:
        ...
        is_valid, error_msg = validate_timestamp_range(
            payment_header_timestamp_ns,
            reservation.start_timestamp,
            reservation.end_timestamp
        )
        if not is_valid:
        ...
        if not is_within_advance_window(
            current_time_s,
            reservation.start_timestamp,
            config.reservation_advance_window
        ):
            return (f"reservation for quorum {quorum_id} starts at {reservation.start_timestamp}, "
...
def validate_reservation_period(
    ...
    if period_record.usage + symbols_to_charge > bin_limit:
        return False, (f"insufficient reservation capacity: need {symbols_to_charge} symbols, "
                       f"bin has {period_record.usage}/{bin_limit} used"), None
    
    return True, "", period_record


def check_quorum_in_reservation(
    reservation: ReservedPayment,
    quorum_id: QuorumID
) -> bool:
    """
...
def validate_payment_increment(
    ...
    if increment < min_increment:
        return False, (f"insufficient payment increment: got {increment}, "
                       f"required minimum {min_increment}")
